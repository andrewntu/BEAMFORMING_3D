...
do
	...
	#<<'COMMENT'
	for cor in `ls ${in_path}/cor_${csta}-???_${tt}.ZZ.sac.${freq}`
	do
		pair=`echo $cor | awk -F "cor_" '{print $2}' | awk -F_ '{print $1}'`
		cor_ZE=`ls ${in_path}/cor_${pair}_${tt}.ZE.sac.${freq}`
		cor_ZN=`ls ${in_path}/cor_${pair}_${tt}.ZN.sac.${freq}`
	...
	#tmin=`awk '{print $1}' ${csta}-${csta}_ZZ.tmp | minmax -C | awk '{print$1}'`
	#tmax=`awk '{print $1}' ${csta}-${csta}_ZZ.tmp | minmax -C | awk '{print$2}'`
	tmin=-2.0
	tmax=2.0
	amp_max_ZZ=`awk '{print $2*$2}' ${csta}-${csta}_ZZ.tmp | minmax -C | awk '{print sqrt($2)}'`
	amp_max_ZE=`awk '{print $2*$2}' ${csta}-${csta}_ZE.tmp | minmax -C | awk '{print sqrt($2)}'`
	amp_max_ZN=`awk '{print $2*$2}' ${csta}-${csta}_ZN.tmp | minmax -C | awk '{print sqrt($2)}'`
	amp_max=`echo $amp_max_ZZ $amp_max_ZE $amp_max_ZN | sort -n | awk '{print$3}'`
	#echo $tmin $tmax $amp_max
	#exit
	...
	do
		...
		do
			...
			#amp_ZZ=`grep "$ttt" ${pair}_ZZ.tmp | awk '{print$2}'`
			#amp_ZE=`grep "$ttt" ${pair}_ZE.tmp | awk '{print$2}'`
			#amp_ZN=`grep "$ttt" ${pair}_ZN.tmp | awk '{print$2}'`
			amp_ZZ_norm=`echo $amp_max $amp_ZZ | awk '{printf"%s",$2/$1}'`
			amp_ZE_norm=`echo $amp_max $amp_ZE | awk '{printf"%s",$2/$1}'`
			amp_ZN_norm=`echo $amp_max $amp_ZN | awk '{printf"%s",$2/$1}'`
			#echo $pair $ttt $rec $stlo $stla $amp_ZZ $amp_ZE $amp_ZN $amp_ZZ_norm $amp_ZE_norm $amp_ZN_norm $amp_max
			#exit
			vec=`echo $amp_ZE_norm $amp_ZN_norm | awk '{print sqrt($2**2 + $3**2)}'`
			cos_theta=`echo $amp_ZE_norm $vec | awk '{print ($1)/$2 }'`
			arccos=`echo $amp_ZN_norm $amp_ZE_norm | awk '{print atan2($1,$2)}'`
			if (( $(echo "$amp_ZN_norm >= 0" |bc -l) )); then
				degree=`echo $arccos | awk '{print $1*57.295779513}'`
			else
				degree=`echo $arccos | awk '{print (360 + $1*57.295779513)}'`
			fi
			degree=`echo $arccos | awk '{print $1*57.295779513}'`
			echo $stlo $stla $vec >> plot.list
		...
		vec_max=`awk '{print $4*$4}' ppm.list | minmax -C | awk '{print sqrt($2)}'`
		#echo $vec_max
		awk '{print $1,$2,$3,$4/'${vec_max}'}' ppm.list | psxy -J -R -Svt0.005i/0.05i/0.05i -Gblack -W0.5p -K -O >> $output_ps_file
